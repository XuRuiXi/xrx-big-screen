export default {
  // 圆角
  '@radius-sm': '2px',
  '@radius-md': '4px',

  // 间距
  '@space-sm': '4px',
  '@space-md': '8px',
  '@space-lg': '12px',

  // 字色
  // 标题
  '@color-title': 'rgba(0, 0, 0, 0.85)',
  // 正文
  '@color-base': 'rgba(0, 0, 0, 0.65)',
  // 描述
  '@color-caption': 'rgba(0, 0, 0, 0.45)',
  // 失效
  '@color-disable': 'rgba(0, 0, 0, 0.25)',

  '@color-primary': '#0081ff',
  '@primary-color': '@color-primary',
  '@color-error': '#ff3a00',
  '@color-success': '#0fb745',
  '@color-warning': '#f5a623',

  // 输入框底色
  '@color-background-input': '#e2e9f3',
  // 边框
  '@color-border': '#eeeeee',
  // 页面底色
  '@color-background': '#f2f4f7',
  // 模块底色
  '@color-background-model': '#f3f9ff',

  // checkbox主题修改
  '@checkbox-size': '32px',
  '@checkbox-color': '#00FFE2',
  '@checkbox-check-bg': 'transparent',
  '@checkbox-check-color': '#000',

  // menu主题修改
  '@dropdown-menu-bg': '#042A49CC',
  '@dropdown-font-size': '33.26px',
  '@dropdown-line-height': '33.26px',
};
